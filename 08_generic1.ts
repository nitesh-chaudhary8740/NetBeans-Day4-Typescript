...
const result1 = identity("Hello"); // T is now string
result1.charAt(0)
const result2 = identity<number>(100);     // T is now number
function getLastNumber(arr: number[]): number { return arr[arr.length - 1]; }
function getLastString(arr: string[]): string { return arr[arr.length - 1]; }
function getLastItem<T>(arr: T[]): T {
    return arr[arr.length - 1];
}

const lastNum = getLastItem([1, 2, 3]); // TS knows this is a number
const lastStr = getLastItem(["a", "b", "c"]); // TS knows this is a string
...
const result = merge({ name: "Nitesh" }, { age: 25 });
// TS knows result has BOTH name and age.


//practice set

// Create a Generic Interface ApiResponse<T>:
// It should have data: T.
// It should have status: "success" | "error".
...
const respnseData1 = wrapData(dev1)
const respnseData2 = wrapData("hi this is reponse data")
